...
struct querytree
{
    ...
    querytree(Display* _display, Window& root)
    {
        XQueryTree(_display, root, &root, &parent, &windows, &count);
    }
};

struct errorcheck
{
    errorcheck() : failed(false), lock(mutex)
    {
        self=this;
        XSetErrorHandler(&errorcheck::callback);
    }

    virtual ~errorcheck() {self = NULL;}

    static int  callback (Display* display, XErrorEvent* e)
    {
        if (self!=NULL) self->failed = true; return 0;
    }
...
{
    std::string error = "Window Manager: Error occured. ";
...
{
    std::cout << "WindowManager Plugin Initializing ... ";
    ...
    {
        errorcheck check;
        XSelectInput(m_display->xdisplay(), m_display->root(), SubstructureRedirectMask |
...
void windowmanager::on_button_press(const XButtonEvent& e)
{
    ...
    if (clients.count(e.window)>0)
    {
        ...
        window_geometry wg;
        XGetGeometry(m_display->xdisplay(), nonclient, &wg.root, &wg.x, &wg.y, &wg.width,
                      &wg.height, &wg.bsize, &wg.depth);
...
void windowmanager::on_motion_notify(const XMotionEvent& e)
{
    ...
    if (e.state & Button1Mask)
    {
        if (moving)
        {
            ...
            XMoveWindow(m_display->xdisplay(), e.window, dest_nonclient_pos.v[0],
                            dest_nonclient_pos.v[1]);
        }
        else if (resizing)
        {
            const point2i size_delta(std::max(delta.v[0], -drag_window_size.v[0]),
                                     std::max(delta.v[1], -drag_window_size.v[1]));
            const point2i dest_nonclient_size = drag_window_size + size_delta;
            XResizeWindow(m_display->xdisplay(), e.window, dest_nonclient_size.v[0],
                                                           dest_nonclient_size.v[1]);
            XResizeWindow(m_display->xdisplay(), clients[e.window], dest_nonclient_size.v[0],
                               dest_nonclient_size.v[1]-titlebar_size);
        }
    }
}
...
void windowmanager::decorate(const Window& w, const bool& _existed)
{
    ...
    const Window nonclient = XCreateSimpleWindow(m_display->xdisplay(), m_display->root(),
                                new_x, new_y, new_width, new_height, border_size, border_color,
                                background_color);

    XSelectInput(m_display->xdisplay(), nonclient, SubstructureRedirectMask | 
    ...
    XGrabKey(m_display->xdisplay(), XKeysymToKeycode(m_display->xdisplay(), XK_Tab), Mod1Mask, w,
                false, GrabModeAsync, GrabModeAsync);
    XGrabKey(m_display->xdisplay(), XKeysymToKeycode(m_display->xdisplay(), XK_F4), Mod1Mask, w,
                false, GrabModeAsync, GrabModeAsync);
    XGrabButton(m_display->xdisplay(), Button1, None, w, false, ButtonPressMask, GrabModeSync,
